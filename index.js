...
booky.get("/i/:isbn", async (req, res) => {
  ...
  if (!getSpecified) {
    return res.json({
      error: `The books with isbn ${req.params.isbn} not found`,
    });
  }
  return res.json({ book: getSpecified });
});
...
booky.get("/c/:category", async (req, res) => {
  const getSpecified = await BookModel.findOne({
    category: req.params.category,
  });
  if (!getSpecified)
    return res.json({
      error: `No book found with category ${req.params.category}`,
    });
  return res.json({ books: getSpecified });
});
...
booky.get("/author/books/:isbn", (req, res) => {
  ...
  if (getSpecified.length === 0)
    return res.json({
      Error: `No Authors found with book whose isbn is ${req.params.isbn}`,
    });
...
booky.put("/update/author/:isbn/:authorID", async (req, res) => {
  ...
  return res.json({
    ...
    message: "Succesfully added a author to book",
  });
});
...
booky.delete("/book/delete/author/:isbn/:authorID", async (req, res) => {
  ...
  return res.json({
    Books: updatedBook,
    Authors: updatedAuthor,
    message: "Book successfully deleted from the author",
  });
});
...
booky.delete("/book/delete/publication/:isbn/:pubId", (req, res) => {
  ...
  return res.json({
    message: "Suceesfully deleted an author",
    books: database.books,
    publications: database.publication,
  });
});
